...
function ensureConfig(): string | null {
	if (!KV_REST_API_URL || !KV_REST_API_TOKEN) {
		return 'Missing KV_REST_API_URL or KV_REST_API_TOKEN in environment variables.';
	}
	if (!PRIVATE_ICLOUD_USERNAME || !PRIVATE_ICLOUD_PASSWORD) {
		return 'Missing PRIVATE_ICLOUD_USERNAME or PRIVATE_ICLOUD_PASSWORD in environment variables.';
	}
	return null;
}
...
function normalizeSelection(stored: SelectionConfig | string[] | null): SelectionConfig {
...
export async function fetchAvailabilityEvents(): Promise<{ events: AvailabilityEvent[]; cached: boolean }> {
	...
	const selectionPayload = normalizeSelection(stored);
